...
const number_of_clients = 10; // the number of involved FL clients
...
function generateTrainScores(malicious_trainers) {
  let train_results = [];
...
function generateEvalScores(train_results, malicious_reviewers, malicious_trainers) {
  let eval_pool = [];
...
function run() {
  ...
  console.log('malicious trainers list: ', malicious_trainers);

  const malicious_reviewers = assignMalicious(number_of_clients, malicious_reviewer_ratio);
  console.log('malicious reviewers list: ', malicious_reviewers);

  console.log(`================== TRAINING ==================`);

  const train_results = generateTrainScores(malicious_trainers);
  console.log(train_results);

  console.log(`================== EVALUATION ==================`);

  const eval_results = generateEvalScores(train_results, malicious_reviewers, malicious_trainers);
  for (let i = 0; i < eval_results.length; i++) {
    console.log('reviewer:', eval_results[i].reviewer);
    console.log('scoring:', eval_results[i].scores);
  }

  console.log(`================== CALCULATING TRAINER CONTRIBUTION ==================`);

  const train_scores = groupTrainScorePerTrainer(train_results);
  console.log('train scores per trainer: ', train_scores);

  const eval_scores = groupEvalScorePerReviewer(eval_results);
  console.log('eval scores per reviewer: ', eval_scores);

  const transposed = transposeArray(eval_scores);
  console.log('eval transposed: ', transposed);

  const trimmed = trimArray(transposed);
  console.log('eval trimmed: ', trimmed);

  const quarter_per_trainer = calculateQuartersPerRow(trimmed);
  console.log('eval quarter per trainer: ', quarter_per_trainer);

  const weights = calculateTrainerContribution(train_scores, quarter_per_trainer);
  let agg_result = 0;
  for (let i = 0; i < weights.length; i++) {
    console.log(weights[i] + " * " + train_scores[i])
    agg_result  = agg_result + (weights[i]*train_scores[i])
  }
  console.log('aggregation result:', agg_result)

  // console.log(`================== CALCULATING REVIEWER CONTRIBUTION ==================`);

  // calculateReviewerContribution(eval_scores, quarter_per_trainer);
