...
contract Admin {
    ...
    function addCumScore(address wallet, int score) public {
        clients[wallet].cumScores.push(score);
    }

    function setCumScores(address wallet, int[] memory scores) public {
        clients[wallet].cumScores = scores;
    }

    function getCumScore(address wallet, uint index) public view returns (int) {
        return clients[wallet].cumScores[index];
    }

    function getAllCumScore(address wallet) public view returns (int[] memory) {
        return clients[wallet].cumScores;
    }
    ...
    function calculateCreditScore(address wallet) public view returns (int) {
        ...
        for (uint256 index = 0; index < client.scores.length; index++) {
            if (client.scores[index].time > (block.timestamp - EXPIRE_TIME)) {
                int freshNess = ((int(EXPIRE_TIME) -
                    (int(block.timestamp) - int(client.scores[index].time))) *
                    100) / int(EXPIRE_TIME);
    ...
    function setEvalScores(address worker, address reviewer, int[] memory arr) public {
        clients[worker].evalScores[reviewer] = arr;
    }
    function addEvalScores(address worker, address reviewer, int score) public {
        clients[worker].evalScores[reviewer].push(score);
    }

    function calculateWorkerContribution(address wallet) public returns (int) {
        int score = clients[wallet].cumScores[
            clients[wallet].cumScores.length - 1
        ];
        int[] memory scores;
        Client storage client = clients[wallet];
        for (uint256 i = 0; i < addresses.length; i++) {
            if (addresses[i] != wallet) {
                scores = client.evalScores[addresses[i]];
                int[] memory sortedScores = sort(scores);
                int[] memory quarters = new int[](3);
                (quarters[0], quarters[1], quarters[2]) = getQuarters(
                    sortedScores
                );
    ...
    //                     _scores[j] = (
    //                         clients[addresses[j]].evalScores[addresses[i]]
    //                     );
    ...
    //             int interQtr = quarters[2] - quarters[0];
    //             int reviewerScoreOnThisClient = clients[reviewer].evalScores[
    //                 addresses[i]
    //             ];
    //             if (
    //                 isReviewerScoreTooHigh(
    //                     reviewerScoreOnThisClient,
    //                     quarters,
    //                     interQtr
    //                 )
    //             ) {
    ...
    function isReviewerScoreTooHigh(
        int reviewerScoreOnThisClient,
        int[] memory quarters,
        int interQtr
    ...
    function isReviewerScoreTooLow(
        int reviewerScoreOnThisClient,
        int[] memory quarters,
        int interQtr
    ) public pure returns (bool) {
    ...
    function getCreditEvent(
        ...
        if (index < clients[wallet].scores.length)
            return (
                clients[wallet].scores[index].score,
                clients[wallet].scores[index].time
            );
        return (0, 0);
    }
    ...
    function getClient(
        ...
        public
        view
        returns (uint256, uint256, uint256, uint[] memory, string memory)
    {
        return (
            clients[wallet].level,
            clients[wallet].experience,
            clients[wallet].experienceNext,
            clients[wallet].events,
            clients[wallet].publicKey
        );
    }

    function getMedian(int[] memory arr) public pure returns (int) {
    ...
    function test(
        int[] memory arr
    ) public view returns (int, int, int, int[] memory, int[] memory) {
        int q1;
        int q2;
        int q3;
    ...
    function normalizeArray(
        ...
        for (uint256 i = 0; i < arr.length; i++) {
            int _max = max(arr);
            int _min = min(arr);
            int normalized = (((arr[i] - _min) * width) / (_max - _min)) +
                start;
    ...
    function reverseNormalizeArray(
        int[] memory arr
    ) public pure returns (int[] memory) {
